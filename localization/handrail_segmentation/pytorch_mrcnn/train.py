...
def main():
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, batch_size=params.hyperparams["batch_size"], shuffle=True, num_workers=params.hyperparams["num_workers"],
    ...
    optimizer = torch.optim.SGD(optimizer_params, lr=params.hyperparams["learning_rate"],
                                momentum=params.hyperparams["momentum"], weight_decay=params.hyperparams["weight_decay"])
    ...
    for epoch in range(num_epochs):
        ...
        for batch_i, (images, targets) in enumerate(tqdm.tqdm(data_loader, desc=f"Training Epoch {epoch}")):

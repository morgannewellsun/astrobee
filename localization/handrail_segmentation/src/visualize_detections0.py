...
class VisualizeImageDetections(object):
    def __init__(self):
        ...
        # Publishers
        self.image_sub = rospy.Subscriber("det/features/annotated_img", ROSImage, self.imageCb, queue_size = 1, buff_size = 2**24)


    def imageCb(self, img_msg):
        img = np.frombuffer(img_msg.data, dtype=np.uint8).reshape(img_msg.height, img_msg.width, -1)[:,:,::-1]
